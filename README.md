...
## コンテナジオメトリと設定永続化（実機確認 2026-07-24）

- **【重要】コンテナのwidth/x等に小数を渡すと `createStartUpPageContainer` が `1`（invalid）で拒否される**。code=1は「既存ページあり」とは限らない——文字幅推定にASCII 8.5pxを使って幅207.5pxを送っただけで、プロトタイプが「恒久的に壊れた」ように見えた（グラスリセット・iPhone再起動でも直らず＝毎回こちらが不正値を送っていただけ）。**ジオメトリは必ずMath.ceil等で整数化**すること。丸1日を失った教訓。
- **テキストコンテナに文字揃えプロパティは無い**（xPosition/yPosition/width/height/border系/paddingLength/contentのみ）。右揃え・中央揃えは「幅を文字列の推定実幅ぴったりにしてコンテナ位置で寄せる」疑似実装になる。ホストフォントの実測: **ASCII≈9.5px/漢字≈19px**（写真校正の履歴: 11/20→約70px余る、8.5/17→右端が切れる。切れ厳禁なので安全側の9.5/19推奨）。幅が日付で変わる場合はrebuildが必要（深夜に一瞬チラつく）。
- **インストール版ではWebViewのlocalStorageが起動毎に破棄される**（プロトタイプでは残る）。設定の永続化は **`bridge.setLocalStorage(key,value)` / `getLocalStorage(key)`**（ホスト側KVストア）を使う。インスタンス間で共有されるので、多重起動ガード（新インスタンスがトークンを書き→旧が検知して停止）にも使える。R1リング再起動で旧インスタンスが残り「猫が2匹点滅」する問題（約2.2Hzで旧最終フレームと交互表示）はこれで対策。
- プロトタイプモードとインストール版は同一package_idでも**ホスト内で別のページ枠**として扱われる（片方だけcreate失敗する切り分けが可能）。
